...
pub async fn setup_pool_from_env() -> Result<MySqlPool, sqlx::Error> {
    dotenv().map_err(|e| sqlx::Error::Configuration(e.into()))?;
    
    info!("Fecthing DB variables from evinronment");
    let host = get_env_var("DB_HOST")?;
    let port: u16 = get_env_var("DB_PORT")?
        .parse()
        .map_err(|e: ParseIntError| Error::Configuration(e.into()))?;
    let username = get_env_var("DB_USERNAME")?;
    let password = get_env_var("DB_PASSWORD")?;
    let database_name = get_env_var("DB_NAME")?;

    let pool = init_pool(&host, port, &username, &password, &database_name).await?;
    info!("Running DB migrations");
    sqlx::migrate!("./db").run(&pool).await?;
    
    info!("DB setup completed successfully");
    Ok(pool)
}
